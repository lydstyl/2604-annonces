...
**Google Maps:**
- Maps are pre-configured using custom iframe embed URLs in listing data
- NO API key needed - maps work out of the box
- To customize a map: Get embed URL from Google Maps "Share" → "Embed a map"
- Add the iframe `src` URL to the `mapEmbedUrl` field in [lib/listings.ts](lib/listings.ts)
